...
features of goroutines and channels to achieve
*/

/*
our state will be owned by a single goroutine.
 This will guarantee that the data is never
 corrupted with concurrent access. In order
 to read or write that state, other goroutines
...
func main() {
	...
	*/
	...
	/*
		This goroutine repeatedly selects on the
		reads and writes channels, responding to
		 requests as they arrive. A response is executed
		 by first performing the requested operation
		 and then sending a value on the
		  response channel resp to indicate success
	*/
	go func() {
		...
		for {
			...
			case write := <-writes:
				...
				fmt.Println("[write.key:", write.key)
				fmt.Println("write.val:", write.val)
				write.resp <- true
			}
		}
	}()
	/*
		100 goroutines to issue reads to the
